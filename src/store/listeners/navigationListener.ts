...
import {
  ...
  set,
  update
} from '@root/feature/navigation/navigationSlice';
import {
  AddActionType,
  InitActionType,
  RemoveActionType,
  UpdateActionType
} from '@root/feature/navigation/navigationSlice/actions';
...
const addEffect = async (
  _: PayloadAction<AddActionType>,
  listenerApi: ListenerApi
) => {
  const navMessage: NavMessagePayload = {
    type: NavType.ADD,
    payload: { name: '', host: '' }
  };
  const message: Message = { type: Type.NAV, payload: navMessage };
...
const removeEffect = async (
  action: PayloadAction<RemoveActionType>,
  listenerApi: ListenerApi
) => {
  const navMessage: NavMessagePayload = {
